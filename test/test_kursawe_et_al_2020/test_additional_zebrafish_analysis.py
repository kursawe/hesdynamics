...
#                                                 these_results_after[:,4]<150))))
                ...
                if len(these_indices>0):
                    for item in these_indices:
                        ...
                        coherences_after.append(these_results_after[item,5])
...
#         new_lengthscales = np.load(saving_path + '_new_lengthscales.npy')
        ...
        if plot_option == 'boxplot':
            this_data_frame = pd.DataFrame(np.column_stack((np.power(results_before_change[:,1],2)*results_before_change[:,0],
                                                            np.power(results_after_change[:,1],2)*results_after_change[:,0])),
                                           columns = ['CTRL','MBS'])
            this_data_frame.boxplot(ax = axes[0,0])
        else:
            for parameter_index in range(parameters_before_change.shape[0]):
                this_axes.plot([0,1],
                         [np.power(parameters_before_change[parameter_index,1],2)*parameters_before_change[parameter_index,0],
                          np.power(parameters_after_change[parameter_index,1],2)*parameters_after_change[parameter_index,0]],
                         color = 'black',
                         alpha = 0.01)
#                          alpha = 1.0)
                this_axes.set_xticks([0,1])
                this_axes.set_xticklabels(['CTRL','MBS'])
        this_axes.set_ylabel('Fano factor')
...
#                 this_alpha = 1.0
        ...
        this_axes = axes[2,1]
        if plot_option == 'boxplot':
            this_data_frame = pd.DataFrame(np.column_stack((results_before_change[:,-2]/np.power(results_before_change[:,1]*
                                                                                                 results_before_change[:,0],2),
        ...
        for parameter_index in range(results_before_change.shape[0]):
            total_count+=1
            value_before = results_before_change[parameter_index,-2]/np.power(results_before_change[parameter_index,1]*
                                                                                             results_before_change[parameter_index,0],2)
            value_after = results_after_change[parameter_index,-2]/np.power(results_after_change[parameter_index,1]*
...
#                 this_alpha = 1.0
            ...
            if plot_option == 'lines':
                this_axes.plot([0,1],
                         [results_before_change[parameter_index,-2]/np.power(results_before_change[parameter_index,1]*
                                                                                                 results_before_change[parameter_index,0],2),
                          results_after_change[parameter_index,-2]/np.power(results_after_change[parameter_index,1]*
                                                                                                results_after_change[parameter_index,0],2)],
                         color = this_color,
                         alpha = this_alpha)
        this_axes.set_title(r'$P_{up}$=' + '{:.2f}'.format(up_count/total_count))
...
#         dual_sweep_results = np.load(os.path.join(os.path.dirname(__file__),'output','zebrafish_dual_sweeps_standard_large_complete_matrix.npy'))
        ...
        list_of_indices = []
        corresponding_proportions = []
        corresponding_proportions_indices = []
        periods_before = []
        periods_after = []
        coherences_after = []
        fluctuation_rate_increases = []
        relative_noise_increases = []
        for translation_index, translation_change in enumerate(translation_changes):
            for degradation_index, degradation_change in enumerate(degradation_changes):
                these_results_after = dual_sweep_results[:,
                                                         degradation_index,
...
#                                                 these_results_after[:,4]<150))))
                ...
                if len(these_indices>0):
                    for item in these_indices:
                        list_of_indices.append(item)
                        corresponding_proportions.append((degradation_change, translation_change))
                        corresponding_proportions_indices.append((degradation_index, translation_index))
                        periods_before.append(my_posterior_results[item,2])
                        periods_after.append(these_results_after[item,4])
                        coherences_after.append(these_results_after[item,5])
                        fluctuation_rate_increases.append((these_results_after[item,-1]-fluctuation_rates_before[item])/
                                                        fluctuation_rates_before[item])
                        relative_noise_increases.append((relative_noise_after[item] - relative_noise_before[item])/
                                                        relative_noise_before[item])
#                         if item == 71792:
...
#         maximal_reference_index = fluctuation_rate_order[-2]
        maximal_reference_index = np.argmax(fluctuation_rate_increases)
#         maximal_reference_index = np.argmax(relative_noise_increases)
        print('reference index')
        print(maximal_reference_index)
        reference_index = maximal_reference_index
        example_index = list_of_indices[maximal_reference_index]
        ...
        print('degradation proportion')
        print(corresponding_proportions[reference_index][0])
        print('translation proportion')
        print(corresponding_proportions[reference_index][1])
...
#         plt.savefig(os.path.join(os.path.dirname(__file__),'output','zebrafish_' + change + '_degradation_examples.pdf'))
    ...
    ### POWER SPECTRA
        example_mrna_before, example_protein_traces_before = hes5.generate_multiple_langevin_trajectories(400*5,
                                                                  1500, #duration
                                                                  example_parameter_before[2], #repression_threshold,
                                                                  example_parameter_before[4], #hill_coefficient,
                                                                  example_parameter_before[5], #mRNA_degradation_rate,
...
#                                                                 2000)
        ...
        example_mrna_traces_after, example_protein_traces_after = hes5.generate_multiple_langevin_trajectories(400*5,
                                                                  1500, #duration
                                                                  example_parameter_after[2], #repression_threshold,
                                                                  example_parameter_after[4], #hill_coefficient,
                                                                  example_parameter_after[5], #mRNA_degradation_rate,
...
#                                                                 2000)
        ...
        last_right_index = np.max(np.where(power_spectrum_after[:,0]<1.0/6.0))
        last_right_index = np.max(np.where(power_spectrum_after[:,0]<1.0/6.0))
        print('power spectrum analysis')
        print(first_left_index)
        print(1.0/40)
        print(power_spectrum_before[first_left_index,0])
        print(power_spectrum_before[first_left_index,1])
        integration_axis = np.hstack(([frequency_cutoff], power_spectrum_before[first_left_index:last_right_index+1,0]))
        print(integration_axis)
        power_spectrum_interpolation = scipy.interpolate.interp1d(power_spectrum_before[:,0], power_spectrum_before[:,1])
        interpolation_values = power_spectrum_interpolation(integration_axis)
        print(interpolation_values)
        noise_weight = 0
        for index, value in enumerate(interpolation_values[:-1]):
            noise_weight+= value*(integration_axis[index+1]-integration_axis[index])
#         noise_weight = np.trapz(interpolation_values, integration_axis)
        print(noise_weight)

        frequency_cutoff = 1.0/40.0
        first_left_index = np.min(np.where(power_spectrum_after[:,0]>frequency_cutoff))
        last_right_index = np.max(np.where(power_spectrum_after[:,0]<1.0/6.0))
        print('power spectrum analysis after')
        print(first_left_index)
        print(1.0/40)
        print(power_spectrum_after[first_left_index,0])
        print(power_spectrum_after[first_left_index,1])
        integration_axis = np.hstack(([frequency_cutoff], power_spectrum_after[first_left_index:last_right_index+1,0]))
        print(integration_axis)
        power_spectrum_interpolation = scipy.interpolate.interp1d(power_spectrum_after[:,0], power_spectrum_after[:,1])
        interpolation_values = power_spectrum_interpolation(integration_axis)
        noise_weight = 0
        for index, value in enumerate(interpolation_values[:-1]):
            noise_weight+= value*(integration_axis[index+1]-integration_axis[index])
...
#         noise_weight = np.trapz(interpolation_values, integration_axis)
        ...
        print('mean before')
        print(np.mean(example_protein_traces_before[:,1:]))
        print(my_posterior_results[list_of_indices[maximal_reference_index],0])
        print('mean after')
        print(np.mean(example_protein_traces_after[:,1:]))
        print(dual_sweep_results[list_of_indices[maximal_reference_index],
                                 corresponding_proportions_indices[maximal_reference_index][0],
                                 corresponding_proportions_indices[maximal_reference_index][1],2])
        print('coherence_before')
        print(my_posterior_results[list_of_indices[maximal_reference_index],3])
        print(my_posterior_results[list_of_indices[maximal_reference_index],2])
        print(coherence_before)
        print(period_before)
        print(periods_before[maximal_reference_index])
        print('coherence_after')
        print('period according to matrix')
        print(dual_sweep_results[list_of_indices[maximal_reference_index],
                                 corresponding_proportions_indices[maximal_reference_index][0],
                                 corresponding_proportions_indices[maximal_reference_index][1],5])
        print('coherence according to matrix')
        print(dual_sweep_results[list_of_indices[maximal_reference_index],
                                 corresponding_proportions_indices[maximal_reference_index][0],
                                 corresponding_proportions_indices[maximal_reference_index][1],4])
        print('calculated coherence')
        print(coherence_after)
        print('calculated period')
        print(period_after)
        print('saved period')
        print(periods_after[maximal_reference_index])
        print('saved coherence')
        print(coherences_after[maximal_reference_index])
        print('power_spectrum_weight before')
        print(my_posterior_results[list_of_indices[maximal_reference_index],-2])
        print(hes5.calculate_noise_weight_from_power_spectrum(power_spectrum_before))
        print(relative_noise_before[list_of_indices[maximal_reference_index]])
        print('power_spectrum_weight after')
        print(dual_sweep_results[list_of_indices[maximal_reference_index],
                                 corresponding_proportions_indices[maximal_reference_index][0],
                                 corresponding_proportions_indices[maximal_reference_index][1],-2])
        print(hes5.calculate_noise_weight_from_power_spectrum(power_spectrum_after))
#         print(relative_noise_before[list_of_indices[maximal_reference_index]])
#         import pdb; pdb.set_trace()
        ...
        plt.plot(smoothened_power_spectrum_before[:,0],
                smoothened_power_spectrum_before[:,1], lw = 0.5 , color = 'black')
        plt.plot(power_spectrum_after[:,0],
                 power_spectrum_after[:,1], lw = 0.5, label = 'MBS')
        plt.plot(smoothened_power_spectrum_after[:,0],
                smoothened_power_spectrum_after[:,1], lw = 0.5, color = 'black')
        plt.axvline(1./40)
        plt.axvline(1./6.0)
        plt.legend()
        plt.xlim(0,0.1)
        plt.tight_layout()
        plt.savefig(os.path.join(os.path.dirname(__file__),'output','dual_change_power_spectra.pdf'))
...
#         plt.plot(gamma, gamma)
    ...
    def test_compare_quality_and_coherence(self):
...
#         saving_path = os.path.join(os.path.dirname(__file__), 'output','sampling_results_zebrafish_delay_large')
    ...
    def xest_make_time_delay_variation(self):
        number_of_parameter_points = 30
        number_of_trajectories = 400
#         number_of_parameter_points = 2
#         number_of_trajectories = 2
...
#         saving_path = os.path.join(os.path.dirname(__file__), 'data','sampling_results_extended')
    ...
    def xest_reconstruct_other_dual_parameter_variation_matrix(self):
#         model = 'standard'
#         model = 'standard_large'
        ...
        all_sub_matrices = []
        for quadrant_index in range(17,39):
            this_saving_path = saving_path_root + str(quadrant_index) + '.npy'
            all_sub_matrices.append(np.load(this_saving_path))

        this_full_matrix = np.zeros((len(all_sub_matrices[0]),8,33,14))
        for parameter_index in range(len(all_sub_matrices[0])):
            this_upper_matrix = all_sub_matrices[0][parameter_index]
            for submatrix_index in range(1,11):
                this_upper_matrix = np.hstack((all_sub_matrices[submatrix_index][parameter_index],this_upper_matrix))
            this_lower_matrix = all_sub_matrices[11][parameter_index]
...
#         plt.gca().text(x_coord, y_coord, 'A', transform=plt.gca().transAxes)
    ...
    def xest_plot_level_curves(self):
...
#         axes.set_xlim(left = -3, right = 3)
    ...
    def xest_plot_coherence_curves_time_delay(self):

        my_figure = plt.figure( figsize = (2.5, 1.9) )

        my_degradation_sweep_results = np.load(os.path.join(os.path.dirname(__file__), 'output',
                                                        'zebrafish_relative_sweeps_time_delay.npy'))
#                                                           'repeated_degradation_sweep.npy'))
#         print(my_degradation_sweep_results[0,:,0])
#         my_filtered_indices = np.where(np.logical_and(my_degradation_sweep_results[:,9,4]>0.1,
#                                        np.logical_and(my_degradation_sweep_results[:,9,4]<0.6,
...
#         plt.gca().text(x_coord, y_coord, 'A', transform=plt.gca().transAxes)
        ...
        plt.tight_layout()
        file_name = os.path.join(os.path.dirname(__file__),
                                 'output','zebrafish_coherence_curves_time_delay')

        plt.savefig(file_name + '.pdf', dpi = 600)
        plt.savefig(file_name + '.png', dpi = 600)

    def xest_plot_period_curves_time_delay(self):
...
#         plt.gca().text(x_coord, y_coord, 'A', transform=plt.gca().transAxes)
        ...
        plt.tight_layout()
        file_name = os.path.join(os.path.dirname(__file__),
                                 'output','zebrafish_period_curves_time_delay')

        plt.savefig(file_name + '.pdf', dpi = 600)
        plt.savefig(file_name + '.png', dpi = 600)

    def xest_plot_level_curves_time_delay(self):
...
#         plt.gca().text(x_coord, y_coord, 'A', transform=plt.gca().transAxes)
        ...
        plt.tight_layout()
        file_name = os.path.join(os.path.dirname(__file__),
                                 'output','zebrafish_level_curves_time_delay')

        plt.savefig(file_name + '.pdf', dpi = 600)
        plt.savefig(file_name + '.png', dpi = 600)

    def xest_plot_cov_curves_time_delay(self):
...
#         plt.gca().text(x_coord, y_coord, 'A', transform=plt.gca().transAxes)
        ...
        plt.tight_layout()
        file_name = os.path.join(os.path.dirname(__file__),
                                 'output','zebrafish_cov_curves_time_delay')

        plt.savefig(file_name + '.pdf', dpi = 600)
        plt.savefig(file_name + '.png', dpi = 600)

    def xest_different_multiplicative_noise_strengths(self):

        #Let's make a standard trajectory  and then three panels with the same paramters and more multiplicative noise
        my_trajectory_1 = hes5.generate_langevin_trajectory( duration = 1500,
        ...
        my_trajectory_2 = hes5.generate_langevin_trajectory( duration = 1500,
                                                         repression_threshold = 31400,
                                                         mRNA_degradation_rate = np.log(2)/30,
                                                         protein_degradation_rate = np.log(2)/90,
                                                         translation_rate = 29,
                                                         ...
                                                         initial_protein = 31400,
                                                         equilibration_time = 1000,
                                                         transcription_noise_amplification = 2.0)

        my_trajectory_10 = hes5.generate_langevin_trajectory( duration = 1500,
                                                         repression_threshold = 31400,
                                                         mRNA_degradation_rate = np.log(2)/30,
                                                         protein_degradation_rate = np.log(2)/90,
                                                         translation_rate = 29,
                                                         ...
                                                         initial_protein = 31400,
                                                         equilibration_time = 1000,
                                                         transcription_noise_amplification = 10.0)

        my_trajectory_100 = hes5.generate_langevin_trajectory( duration = 1500,
                                                         repression_threshold = 31400,
                                                         mRNA_degradation_rate = np.log(2)/30,
                                                         protein_degradation_rate = np.log(2)/90,
                                                         translation_rate = 29,
                                                         ...
                                                         initial_protein = 31400,
                                                         equilibration_time = 1000,
                                                         transcription_noise_amplification = 100.0)

        my_figure = plt.figure(figsize = (6,4.5))
        plt.subplot(221)
        plt.plot(my_trajectory_1[:,0],
                 my_trajectory_1[:,1]*1000, label = 'mRNA*1000', color = 'black')
        plt.plot(my_trajectory_1[:,0],
                 my_trajectory_1[:,2], label = 'Hes protein', color = 'black', ls = '--')
        plt.xlabel('Time')
        plt.ylabel('Copy number')
        plt.subplot(222)
        plt.plot(my_trajectory_2[:,0],
                 my_trajectory_2[:,1]*1000, label = 'mRNA*1000', color = 'black')
        plt.plot(my_trajectory_2[:,0],
                 my_trajectory_2[:,2], label = 'Hes protein', color = 'black', ls = '--')
        plt.xlabel('Time')
        plt.ylabel('Copy number')
        plt.subplot(223)
        plt.plot(my_trajectory_10[:,0],
                 my_trajectory_10[:,1]*1000, label = 'mRNA*1000', color = 'black')
        plt.plot(my_trajectory_10[:,0],
                 my_trajectory_10[:,2], label = 'Hes protein', color = 'black', ls = '--')
        plt.xlabel('Time')
        plt.ylabel('Copy number')
        plt.subplot(224)
        plt.plot(my_trajectory_100[:,0],
                 my_trajectory_100[:,1]*1000, label = 'mRNA*1000', color = 'black')
        plt.plot(my_trajectory_100[:,0],
                 my_trajectory_100[:,2], label = 'Hes protein', color = 'black', ls = '--')
        plt.xlabel('Time')
        plt.ylabel('Copy number')
        plt.tight_layout()
        my_figure.savefig(os.path.join(os.path.dirname(__file__),
...
#         gillespie_results = hes5.calculate_summary_statistics_at_parameters(posterior_samples, model = 'gillespie_sequential')
        ...
        np.save(saving_path + '.npy', these_repeated_results)
#         gillespie_results = np.load(saving_path + '.npy')

        repeated_standard_deviation = these_repeated_results[:,1]

        error_ratios = posterior_results[:,1]/these_repeated_results[:,1]

        print('number of outlier samples')
        print(np.sum(error_ratios>1.1))

        plt.figure(figsize = (4.5,2.5))
        plt.scatter(repeated_standard_deviation, posterior_results[:,1], s = 0.5)
        plt.plot(np.linspace(0.05,0.16,3),np.linspace(0.05,0.16,3))
